...
##################################################
DEVELOPER_KEY = '본인의 API key를 입력하세요'    ###### 
##################################################
...
def youtube_search_query(options, pagetoken):
...
def comment(v_id_list):
    ...
    cdf = pd.DataFrame(zip(vid_list_c, comment_list, c_id_list, reply_list, c_time_list, c_author_list, like_list, ori_rep), columns=['video_id','comment','comment_id', 're_reply','comment_time', 'author', 'c_like', 'ori_rep'])
